...
try {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM property_types WHERE slug = ?");
    $stmt->execute([$slug]);
    ...
    if ($stmt->rowCount() > 0) {
        echo json_encode(['success' => true, 'message' => 'Property type added successfully']);
    } else {
        http_response_code(500);
        echo json_encode(['success' => false, 'error' => 'Failed to add property type']);
    }
} catch (Exception $e) {
    http_response_code(500);
